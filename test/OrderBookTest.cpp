...
/// Used to represent the expected attributes of an order  in test validations.
/// It simplifies the order details to essential elements like id, visible size,
/// and price for easy comparison in test assertions.
struct OrderState {
  int id;
  int visible_size;
  PriceType price;
...
class BookTest : public ::testing::Test {
 protected:
  ...
  void ValidateOrder(const Order* order, const OrderState& expected) {
  ...
  void ValidateOrders(const std::vector<const Order*>& actual_orders,
                      const std::vector<OrderState>& expected_orders) {
...
TEST_F(BookTest, AggressiveIceberg) {
  ...
  std::vector<OrderState> expected_bids{
      {6, 10000, 100}, {2, 50000, 99}, {5, 25500, 98}};
  std::vector<OrderState> expected_asks{{1, 20000, 101}};
...
TEST_F(BookTest, LimitAndAggressiveIceberg) {
  ...
  std::vector<OrderState> expected_bids;
  std::vector<OrderState> expected_asks{{100345, 10000, 5103}};
...
TEST_F(BookTest, ExhaustIcebergPeak) {
  ...
  std::vector<OrderState> expected_bids{{100322, 10000, 5103}};
  std::vector<OrderState> expected_asks;
...
TEST_F(BookTest, RemovingAskPriceLevel) {
  ...
  std::vector<OrderState> expected_bids{{100346, 1000, 5103}};
  std::vector<OrderState> expected_asks{{100351, 50, 5104}};
...
TEST_F(BookTest, RemovingBidPriceLevel) {
  ...
  std::vector<OrderState> expected_bids{{100351, 50, 99}};
  std::vector<OrderState> expected_asks{{100346, 50, 100}};
...
TEST_F(BookTest, AggressiveBidIcebergWithLesserTotal) {
  ...
  std::vector<OrderState> expected_bids{{2, 200, 100}};
  std::vector<OrderState> expected_asks;
...
TEST_F(BookTest, AggressiveAskIcebergWithLesserTotal) {
  ...
  std::vector<OrderState> expected_bids;
  std::vector<OrderState> expected_asks{{2, 200, 100}};
